-- Add queue assignment option to fulfillment task templates
ALTER TABLE public.service_catalog_item_tasks
  ADD COLUMN IF NOT EXISTS default_queue_id UUID REFERENCES public.helpdesk_queues(id) ON DELETE SET NULL;

-- Update spawn function to honor queue_id when assignee is a queue
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.helpdesk_tickets (
      ...
      assignee_id, queue_id, parent_ticket_id, created_by, status, metadata
    ) VALUES (
      ...
      v_task.default_assignee_id, v_task.default_queue_id,
      _parent_ticket_id, v_parent.reporter_user_id, 'open',
      jsonb_build_object(
    ...
    VALUES (_parent_ticket_id, v_org, NULL, 'catalog_task_spawned',
            jsonb_build_object('child_ticket_id', v_new_id, 'task_id', v_task.id, 'step', v_task.step_order, 'title', v_task.title));
  END LOOP;
