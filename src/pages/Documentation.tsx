...
const platformFeatureGroups = [
  ...
  },
  {
    ...
    features: [
      ...
      {
        name: "SSO Setup & JIT Provisioning",
        icon: KeyRound,
        summary: "SAML/OIDC SSO with self-service request flow, platform-admin queue, and Just-in-Time provisioning on first login.",
        capabilities: [
          "Self-service config request from org admins",
          "Platform admins approve and configure IdP metadata",
          "JIT auto-attaches users to the matching org on first login",
          "Default access level + default custom roles per SSO config",
          "Recent JIT activity feed (last 10 sign-ins, status & errors)",
          "Default roles also applied to invited / bulk-imported users when their email domain matches",
        ],
        permission: "can_manage_integrations",
      },
      {
        name: "User Provisioning & Bulk Import",
        icon: Users,
        summary: "Five paths to onboard users — invite, bulk CSV, migration mapping, reconciliation, SSO JIT — all writing through the same access model.",
        capabilities: [
          "CSV bulk import with email, names, access level and custom-role UUIDs",
          "Migration Wizard Step 3 maps source users with optional auto-invite",
          "Reconcile Migrated Users repairs ownership after invites are accepted",
          "Org-scoped for Org Admins; cross-org targeting for Platform Admins",
          "Billable seats tracked via is_billable_tier() and surfaced on Licenses",
        ],
